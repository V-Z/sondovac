...
while getopts "hvlrpeo:c:x:z:b:d:y:k:" START; do
	case "${START}" in
		...
		y)
			BLATIDENT="${OPTARG}"
			# Check if provided value makes sense
			if [[ "${BLATIDENT}" =~ ^[0-9]+$ ]] && [ "${BLATIDENT}" -ge 85 ] && [ "${BLATIDENT}" -le 95 ]; then
...
REQUIREDCOLS="Required columns in ${TSVLIST} are \"# Sequences\", \"% Pairwise Identity\", \"Description\", \"Mean Coverage\", \"Name\" and \"Sequence Length\". Please, export the TSV file again."
echo
...
if grep -E -q "^# Sequences[[:blank:]]+% Pairwise Identity[[:blank:]]+Description[[:blank:]]+Mean Coverage[[:blank:]]+Name[[:blank:]]+Sequence Length$" "${TSVLIST}"
	then
...
MINLOCUSLENGTHN=$((MINLOCUSLENGTH - 1))
